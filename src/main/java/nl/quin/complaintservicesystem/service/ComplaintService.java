...
public class ComplaintService {
    ...
    public void updateComplaint(long id, Complaint complaint) {
        if (!complaintRepository.existsById(id)) { throw new UserNotFoundException(); }
        Complaint storedComplaint = complaintRepository.findById(id).orElse(null);
        storedComplaint.setOrderNumberCustomerLog(complaint.getOrderNumberCustomerLog());
        storedComplaint.setReceiptValid(complaint.getReceiptValid());
        storedComplaint.setName(complaint.getName());
        storedComplaint.setRePrint(complaint.getRePrint());
        storedComplaint.setExtraCostsEstimate(complaint.getExtraCostsEstimate());
        storedComplaint.setProductionCommentary(complaint.getProductionCommentary());
        complaintRepository.save(complaint);
    }
    ...
    public void partialUpdateComplaint(long id, Map<String, String> fields) {
        ...
        for (String field : fields.keySet()) {
            switch (field) {
                case "order-number-customer-log":
                    storedComplaint.setOrderNumberCustomerLog((String) fields.get(field));
                    break;
                case "receipt_valid":
                    storedComplaint.setReceiptValid((String) fields.get(field));
                    break;
                case "file_name":
                    storedComplaint.setName((String) fields.get(field));
                    break;
                case "re_print":
                    storedComplaint.setRePrint((String) fields.get(field));
                    break;
                case "extra_costs_estimate":
                    storedComplaint.setExtraCostsEstimate((String) fields.get(field));
                    break;
                case "production_commentary":
                    storedComplaint.setProductionCommentary((String) fields.get(field));
                    break;
